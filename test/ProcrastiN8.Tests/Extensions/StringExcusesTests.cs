...
{
    [Fact]
    public async Task ToExcuse_Returns_Default_Excuse_When_No_Provider()
    {
        // arrange
        var context = "Unit tests";

        // act
        var excuse = await context.ToExcuse();
    ...
    [Fact]
    public async Task ToExcuse_Uses_ExcuseProvider_When_Provided()
    {
        // arrange
        var context = "Integration tests";
        var provider = Substitute.For<IExcuseProvider>();
        provider.GetExcuseAsync().Returns("The CI pipeline is on a coffee break.");

        // act
        var excuse = await context.ToExcuse(provider);
    ...
    [Fact]
    public async Task ToExcuseWithPrefix_Prepends_Prefix_And_Uses_Provider()
    {
        // arrange
        var context = "Documentation";
        var prefix = "[BLOCKED] ";
        var provider = Substitute.For<IExcuseProvider>();
        provider.GetExcuseAsync().Returns("Documentation is still being written by Schrödinger's intern.");

        // act
        var excuse = await context.ToExcuseWithPrefix(prefix, provider);
